const GAS_URL = "PASTE_URL_GOOGLE_SCRIPT_DISINI_BOSKU"; 
...
// Pemisahan Sempurna Operasional Bisnis Mandiri Link vs Joki Kilat
...
// Jam Waktu Nyata (Real-time Clock WITA)
setInterval(() => {
    const now = new Date();
...
// Sistem Navigasi Lintas Platform
...
// Jembatan Asinkronus ke Google Apps Script Engine
...
document.getElementById('form-joki').addEventListener('submit', async function(e) {
    ...
    await sendToBackend({ action: "addJoki", ...newData }, document.getElementById('btn-submit-joki'), "MEMPROSES DATA...", "SIMPAN ORDER & PASANG ALARM");
    this.reset();
});
...
document.getElementById('form-tugas').addEventListener('submit', async function(e) {
    ...
    await sendToBackend({ action: "addTugas", ...dataBaru }, document.getElementById('btn-submit-tugas'), "MENSINKRONKAN ALARM...", "SIMPAN TUGAS & ALARM");
    this.reset();
});

async function selesaikanTugas(index) {
    if(confirm(`Nyatakan Selesai Tugas ${tugas[index].namaTugas}? Data aktif akan dihapus dari beranda.`)) {
        const targetId = tugas[index].id; // TYPO FIXED DI SINI BOSKU
...
document.getElementById('form-cicilan').addEventListener('submit', async function(e) {
    ...
    await sendToBackend({ action: "addTagihan", ...dataBaru }, document.getElementById('btn-submit-tagihan'), "MENGAMANKAN SERVER...", "SIMPAN TAGIHAN & ALARM");
    this.reset();
});
...
// --- ENGINE PEMBUATAN LAPORAN PDF PREMIUM ---
function unduhPDF(modul, range) {
    ...
    if (modul === 'bisnis') {
        ...
        let dataSumber = bisnis.riwayat;
        if(range === 'hari') dataSumber = bisnis.riwayat.filter(item => item.tanggal === tglSekarang);
        ...
        let dataSumber = jokiLogOmset;
        if(range === 'hari') dataSumber = jokiLogOmset.filter(item => item.tanggal === tglSekarang);
        ...
        let dataSumber = riwayat;
        if(range === 'hari') dataSumber = riwayat.filter(item => item.tanggal === tglSekarang);
...
// --- SISTEM OTENTIKASI KEAMANAN DATA ---
...
function simpanData() { 
    ...
    localStorage.setItem('riwayatClean', JSON.stringify(riwayat)); 
    localStorage.setItem('tugasPro', JSON.stringify(tugas)); 
    localStorage.setItem('cicilanPro', JSON.stringify(cicilan)); 
    localStorage.setItem('akunPenting', JSON.stringify(akunPenting)); 
    localStorage.setItem('bisnisPro', JSON.stringify(bisnis)); 
    localStorage.setItem('jokiPro', JSON.stringify(joki)); 
    localStorage.setItem('jokiLogOmsetPro', JSON.stringify(jokiLogOmset)); 
}
...
function updateUI() {
    ...
    document.getElementById('count-tugas').innerText = tugas.length; 
    document.getElementById('count-joki').innerText = joki.length;
